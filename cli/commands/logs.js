...
const { log, info, warn, col } = require('../lib/colors');
...
function run(args) {
  ...
  // If processes are running, output is already streaming live
  if (showBackend && procs.backend) {
    info('Backend is running — output is streaming live in this terminal.');
  }
  if (showFrontend && procs.frontend) {
    info('Frontend is running — output is streaming live in this terminal.');
  }
