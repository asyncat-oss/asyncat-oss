...
// WebSocket connection for real-time delta sync (optional enhancement)
export class DeltaSyncSocket {
  constructor(noteId, onUpdate) {
    this.noteId = noteId;
  ...
  connect() {
    const wsUrl = API_URL.replace("http", "ws") + `/ws/notes/${this.noteId}`;
    ...
    try {
      ...
      this.ws.onopen = () => {
        secureLogger.info(
          getConnectionStatusMessage("CONNECTED", "delta sync")
        );
        this.reconnectAttempts = 0;
        this.reconnectDelay = 1000;
      };
      ...
      this.ws.onmessage = (event) => {
        try {
          ...
          if (data.type === "delta" && this.onUpdate) {
            this.onUpdate(data);
          }
        } catch (error) {
          secureLogger.error("Error parsing WebSocket message");
        }
      };
      ...
      this.ws.onclose = () => {
        secureLogger.info(
          getConnectionStatusMessage("DISCONNECTED", "delta sync")
        );
        this.attemptReconnect();
      };
    } catch (error) {
      secureLogger.error("Failed to create WebSocket connection");
    }
  }
