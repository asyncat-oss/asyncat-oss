...
function AnswerEvent({ data }) {
  ...
  if (!answer.trim()) return null; // skip empty answers (intermediate rounds)
  ...
  return (
    <div className="rounded-xl border border-indigo-200 dark:border-indigo-700/60 bg-indigo-50/50 dark:bg-indigo-900/10 p-4 mb-2">
      <div className="flex items-center gap-2 mb-3">
        <CheckCircle2 className="w-4 h-4 text-indigo-500 flex-shrink-0" />
        <span className="text-xs font-semibold text-indigo-700 dark:text-indigo-300 uppercase tracking-wide">
          Answer · Round {data?.round}
        </span>
      </div>
      <div className="text-sm text-gray-800 dark:text-gray-200 leading-relaxed prose-sm">
        {blocks.length > 0
...
export default function AgentRunFeed({ events, isRunning, streamingText }) {
