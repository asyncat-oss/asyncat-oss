    ...
    <div className="rounded-lg border border-gray-200 dark:border-gray-700/60 overflow-hidden mb-1.5">
      ...
      {expanded && (
        <div className="border-t border-gray-100 dark:border-gray-700/50 px-3 py-3 bg-gray-50/50 dark:bg-gray-800/20">
          <p className="text-xs text-gray-600 dark:text-gray-300 leading-relaxed">{tool.description || 'No description available'}</p>
          {tool.parameters && (
            <div className="mt-2">
...
export default function AgentToolsView() {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Permission legend */}
      <div className="flex-shrink-0 flex items-center gap-4 px-4 py-2 border-b border-gray-100 dark:border-gray-800 midnight:border-slate-800 bg-gray-50/50 dark:bg-gray-800/30">
        <span className="text-xs text-gray-500 dark:text-gray-400">{tools.length} tools</span>
          ...
          return (
            <div key={category} className="mb-6">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500">{category}</span>
                <span className="text-xs text-gray-300 dark:text-gray-600 ml-auto">{catTools.length}</span>
              </div>
      ...
      <div className="flex-shrink-0 px-4 py-3 border-t border-gray-100 dark:border-gray-800 text-center">
        <p className="text-[11px] text-gray-400 dark:text-gray-600">Tools are called automatically by the agent during tasks</p>
      </div>
    </div>
  );
}
