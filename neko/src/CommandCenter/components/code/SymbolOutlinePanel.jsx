...
const KIND_COLOR = {
  ...
  method: 'text-blue-400 dark:text-blue-300',
  class: 'text-amber-500 dark:text-amber-400',
  interface: 'text-teal-500 dark:text-teal-400',
  type: 'text-purple-500 dark:text-purple-400',
  export: 'text-emerald-500 dark:text-emerald-400',
  import: 'text-gray-400 dark:text-slate-500',
...
export default function SymbolOutlinePanel({ filePath }) {
  ...
  if (!filePath) {
    return (
      <div className="flex h-full items-center justify-center text-xs text-gray-400 dark:text-slate-600">
  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-white dark:bg-slate-950">
      <div className="flex shrink-0 items-center justify-between gap-2 border-b border-gray-100 dark:border-slate-800 px-3 py-2">
        <span className="min-w-0 truncate font-mono text-[10px] text-gray-500 dark:text-slate-500">{filePath}</span>
        <button
          type="button"
      ...
      <div className="min-h-0 flex-1 overflow-y-auto py-1">
        ...
        {!loading && symbols?.length === 0 && (
          <div className="flex h-full items-center justify-center flex-col gap-2 py-8">
            <Box className="h-5 w-5 text-gray-200 dark:text-slate-700" />
            <p className="text-xs text-gray-400 dark:text-slate-600">No symbols found</p>
          </div>
        )}
        {symbols?.map((sym, i) => (
          <div key={i} className="flex items-baseline gap-2 px-3 py-0.5 rounded hover:bg-gray-50 dark:hover:bg-slate-800/60">
            <span className={`shrink-0 font-mono text-[11px] select-none ${kindColor(sym.kind)}`} title={sym.kind}>
              {kindIcon(sym.kind)}
            </span>
            <span className="min-w-0 flex-1 truncate font-mono text-[11px] text-gray-800 dark:text-slate-200">
              {sym.name}
            </span>
            <span className="shrink-0 font-mono text-[10px] tabular-nums text-gray-300 dark:text-slate-700">
              {sym.line}
            </span>
          </div>
        ))}
