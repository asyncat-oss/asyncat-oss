...
import { Activity, Code2, Image, X, History, BookMarked, Globe, RotateCcw, ExternalLink, AlertTriangle, FilePlus, ArrowLeft, List, Cpu, RefreshCw } from 'lucide-react';
...
function formatLoadedModelName(item) {
  return item?.model?.name || item?.model?.filename || item?.activeCheckpoint || item?.runtime || 'Unknown model';
}

function RuntimeStatusPanel({ status = null, loading = false, onRefresh }) {
  const loaded = Array.isArray(status?.loadedModels) ? status.loadedModels : [];
  const activeProvider = status?.activeProvider;
  const comfy = status?.image?.comfyui;
  const simple = status?.image?.simple;

  return (
    <div className="h-full overflow-y-auto p-3">
      <div className="mb-3 flex items-center justify-between gap-2">
        <div className="min-w-0">
          <p className="text-xs font-semibold text-gray-700 dark:text-slate-200 midnight:text-slate-200">Model status</p>
          <p className="mt-0.5 text-[11px] text-gray-400 dark:text-slate-500 midnight:text-slate-500">
            {status?.generatedAt ? new Date(status.generatedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : 'Not refreshed yet'}
          </p>
        </div>
        <button
          type="button"
          onClick={onRefresh}
          disabled={loading}
          className="flex h-7 w-7 shrink-0 items-center justify-center rounded-md text-gray-400 transition-colors hover:bg-gray-100 hover:text-gray-700 disabled:opacity-50 dark:hover:bg-slate-800 dark:hover:text-slate-200 midnight:hover:bg-slate-800 midnight:hover:text-slate-200"
      ...
      {activeProvider && (
        <section className="mb-3 rounded-lg border border-gray-200 bg-white p-3 dark:border-slate-800 dark:bg-slate-950/40 midnight:border-slate-800 midnight:bg-slate-950/60">
          <p className="text-[10px] font-semibold uppercase tracking-wide text-gray-400 dark:text-slate-500 midnight:text-slate-500">Active chat provider</p>
          <p className="mt-1 truncate text-xs font-semibold text-gray-800 dark:text-slate-100 midnight:text-slate-100">{activeProvider.name || activeProvider.providerId}</p>
          <p className="mt-0.5 truncate text-[11px] text-gray-500 dark:text-slate-400 midnight:text-slate-400">{activeProvider.model || 'No model selected'}</p>
        </section>
      )}
      ...
      <section className="mb-3">
        <div className="mb-2 flex items-center justify-between px-0.5">
          <p className="text-[11px] font-medium text-gray-400 dark:text-slate-500 midnight:text-slate-500">
            {loaded.length} loaded runtime{loaded.length !== 1 ? 's' : ''}
          </p>
        </div>
        {loaded.length === 0 ? (
          <div className="rounded-lg border border-dashed border-gray-200 p-4 text-center dark:border-slate-800 midnight:border-slate-800">
            <Cpu className="mx-auto mb-2 h-6 w-6 text-gray-300 dark:text-slate-600 midnight:text-slate-600" />
            <p className="text-xs font-medium text-gray-500 dark:text-slate-400 midnight:text-slate-400">No local model is loaded</p>
          </div>
        ) : (
          <div className="space-y-2">
            {loaded.map((item, index) => (
              <div
                key={`${item.kind || 'runtime'}-${item.runtime || index}-${index}`}
                className="rounded-lg border border-gray-200 bg-white p-3 dark:border-slate-800 dark:bg-slate-950/40 midnight:border-slate-800 midnight:bg-slate-950/60"
              >
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    <p className="truncate text-xs font-semibold text-gray-800 dark:text-slate-100 midnight:text-slate-100">{formatLoadedModelName(item)}</p>
                    <p className="mt-0.5 text-[11px] text-gray-400 dark:text-slate-500 midnight:text-slate-500">
                      {[item.kind?.toUpperCase(), item.runtime, item.status].filter(Boolean).join(' · ')}
                    </p>
                  </div>
                  <span className="rounded-md bg-emerald-50 px-1.5 py-0.5 text-[10px] font-semibold text-emerald-700 dark:bg-emerald-950/30 dark:text-emerald-300 midnight:bg-emerald-950/30 midnight:text-emerald-300">
                    {item.status || 'ready'}
                  </span>
                </div>
                <div className="mt-2 grid grid-cols-2 gap-2 text-[11px] text-gray-500 dark:text-slate-400 midnight:text-slate-400">
                  {item.model?.sizeFormatted && <span className="truncate">Size {item.model.sizeFormatted}</span>}
                  {item.model?.quantization && <span className="truncate">Quant {item.model.quantization}</span>}
                  {item.memory?.rssFormatted && <span className="truncate">RAM {item.memory.rssFormatted}</span>}
                  {item.vram?.usedFormatted && <span className="truncate">VRAM {item.vram.usedFormatted}</span>}
                  {item.vram?.totalGb && <span className="truncate">VRAM total {item.vram.totalGb} GB</span>}
                  {item.port && <span className="truncate">Port {item.port}</span>}
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="space-y-2">
        <div className="rounded-lg border border-gray-200 bg-white p-3 dark:border-slate-800 dark:bg-slate-950/40 midnight:border-slate-800 midnight:bg-slate-950/60">
          <p className="text-[10px] font-semibold uppercase tracking-wide text-gray-400 dark:text-slate-500 midnight:text-slate-500">Image generation</p>
          <p className="mt-1 text-xs font-medium text-gray-700 dark:text-slate-200 midnight:text-slate-200">
            {status?.image?.preferredRuntime || 'Not ready'}
          </p>
          <p className="mt-1 text-[11px] text-gray-500 dark:text-slate-400 midnight:text-slate-400">
            ComfyUI {comfy?.found ? 'ready' : 'offline'} · Simple {simple?.found ? 'ready' : 'missing'}
          </p>
        </div>
      </section>
    </div>
  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-white dark:bg-[#0f1724] midnight:bg-slate-950">
      ...
      <div className="min-h-0 flex-1">
        ...
        {currentTab === 'runtime' && (
          <RuntimeStatusPanel status={runtimeStatus} loading={runtimeStatusLoading} onRefresh={onRuntimeStatusRefresh} />
        )}
      </div>
    </div>
  );
