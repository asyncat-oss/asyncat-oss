...
import { Send, Search, Brain, X, Target, BookOpen, GraduationCap } from 'lucide-react';
...
export const MessageInputV2 = ({
  ...
  const [value, setValue] = useState(() => {
    ...
    try {
      const saved = localStorage.getItem('asyncat_draft_message');
      return saved || "";
    } catch {
      return "";
    }
  ...
  const contextPercent = Math.min(100, Math.round((totalTokens / ctxSize) * 100));
  ...
  const getBorderColor = () => {
    if (!localModel.isReady) return 'border-gray-200 dark:border-gray-700 midnight:border-gray-700';
    if (contextPercent >= 85) return 'border-red-400 dark:border-red-500 midnight:border-red-500';
    if (contextPercent >= 50) return 'border-amber-400 dark:border-amber-500 midnight:border-amber-500';
      ...
      <div className="max-w-4xl mx-auto px-6 py-3">
          ...
          <div className="mb-3 p-3 bg-red-50 dark:bg-red-900/20 midnight:bg-red-900/30 border-l-4 border-red-500 rounded-lg">
            <div className="flex items-start gap-2">
              ...
              <div className="flex-1">
                <h3 className="text-sm font-semibold text-red-800 dark:text-red-200 mb-1">Context window is {contextPercent}% full</h3>
                <p className="text-xs text-red-700 dark:text-red-300 mb-2">
                  The model may stop responding or produce incomplete answers. Start a new conversation for best results.
                </p>
                <button
                  onClick={onReset}
        ...
        {conversationTooLong ? (
          <div className="p-6 bg-orange-50 dark:bg-orange-900/20 midnight:bg-orange-900/30 border border-orange-200 dark:border-orange-800 midnight:border-orange-700 rounded-lg text-center">
            <h3 className="text-lg font-medium text-orange-900 dark:text-orange-100 mb-2">Conversation Too Long</h3>
            <p className="text-sm text-orange-700 dark:text-orange-300 mb-4">Start a new conversation for better performance.</p>
            <button
              onClick={onReset}
          ...
          <form onSubmit={handleSubmit}>
            <div className={`p-4 bg-white dark:bg-gray-800 midnight:bg-gray-900 border-2 rounded-lg transition-colors ${getBorderColor()}`}>
              ...
              {localModel.isReady && (
                <div className="mt-2 flex items-center justify-between text-[10px]">
                  <span className={`${contextPercent > 85 ? 'text-red-600 dark:text-red-400' : contextPercent > 50 ? 'text-amber-600 dark:text-amber-400' : 'text-gray-400 dark:text-gray-500'}`}>
                    {value.length > 0 && `+${inputTokens.toLocaleString()} · `}
                    ~{totalTokens.toLocaleString()} / {ctxSize.toLocaleString()} tokens ({contextPercent}%)
                  </span>
                  <div className="flex items-center gap-2">
                    {draftSaved && value.trim() && (
                      <span className="text-green-600 dark:text-green-400 text-[10px]">✓ Draft saved</span>
                    )}
                    {contextPercent > 85 && (
                      <span className="text-red-500 dark:text-red-400 font-medium">Context nearly full</span>
                    )}
                  </div>
                </div>
              )}
              ...
              <div className="flex items-center justify-between gap-4 pt-3">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <button
                    ...
                    disabled={disabled}
                    title={webSearchEnabled ? 'Web search ON — click to disable' : 'Search the web'}
                  ...
                  <button
                    ...
                    disabled={disabled}
                    title={thinkingEnabled ? 'Thinking ON — click to disable' : 'Enable step-by-step reasoning'}
                  ...
                  >
                    ...
                    {localModel.supportsThinking && !thinkingEnabled && (
                      <span className="absolute -top-1 -right-1 w-2 h-2 rounded-full bg-purple-400" title="Native thinking model" />
                    )}
                  </button>
