...
const EditHabitModal = ({ habit, onClose, onSubmit, isSubmitting = false, selectedProject }) => {
  const [formData, setFormData] = useState({
    ...
    icon: habit.icon || '🎯',
    is_private: habit.is_private !== undefined ? habit.is_private : true
  });
  ...
  // Fun taglines for editing
  const editTaglines = [
    "Making changes because last time wasn't quite delusional enough 🎭",
    "Adjusting the goalposts of your inevitable disappointment 🎯",
    "Fine-tuning your path to self-improvement... or whatever 🛤️",
    "Because the original plan needed more magical thinking ✨",
    "Optimizing your journey to 'I'll start Monday' 📅",
    "Tweaking the metrics of mediocrity 📊",
    "Your habit's getting a makeover (the habit, not you) 💅",
    "Updating settings won't update your commitment 💀",
    "New icon, same procrastination energy 🔄",
    "Rearranging deck chairs on the SS Abandoned Habits 🚢"
  ];

  // Pick a random tagline
  const [randomTagline] = useState(() => editTaglines[Math.floor(Math.random() * editTaglines.length)]);

  // Check if we're using a custom category on mount
  useEffect(() => {
    const predefinedCategories = ['general', 'development', 'communication', 'productivity', 'quality', 'learning', 'collaboration', 'health'];
    if (habit.category && !predefinedCategories.includes(habit.category)) {
  ...
  return (
    <Portal>
      <div 
        className="fixed inset-0 bg-black/40 dark:bg-black/60 midnight:bg-black/70 backdrop-blur-sm flex items-center justify-center z-[60] p-4"
        onClick={(e) => {
      ...
      >
        <div 
          className="bg-white dark:bg-gray-800 midnight:bg-gray-900 rounded-xl shadow-xl w-full max-w-5xl mx-auto max-h-[90vh] flex flex-col border border-gray-200 dark:border-gray-700 midnight:border-gray-800"
        ...
        {success && (
          <div className="mb-5 p-3 bg-green-50 dark:bg-green-900/20 midnight:bg-green-900/10 border border-green-200 dark:border-green-800 midnight:border-green-800/50 rounded-lg flex items-start space-x-2">
            ...
            <p className="text-green-600 dark:text-green-400 midnight:text-green-400 text-sm font-medium">Habit updated successfully!</p>
          </div>
        )}
        ...
        <div className="flex-shrink-0 border-t border-gray-200 dark:border-gray-700 midnight:border-gray-800 px-6 py-4 bg-gray-50 dark:bg-gray-800 midnight:bg-gray-900">
          <div className="flex justify-end space-x-3">
            ...
            >
              ...
              <span>
                {success ? '✓ Updated!' : isSubmitting ? 'Saving...' : 'Save Changes'}
              </span>
            </button>
          </div>
        </div>
