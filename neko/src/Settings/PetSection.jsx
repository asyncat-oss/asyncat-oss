...
const Tile = ({ src, name, selected, onClick }) => (
  ...
  >
    <span className="relative h-14 w-14">
      <img src={src} alt={name} className="h-14 w-14 rounded-2xl object-contain" />
      {selected && (
...
const PetSection = () => {
  const [pet, setPetState] = useState({ enabled: false, sprite: 'default', dataUrl: null });
  ...
  useEffect(() => {
    ...
    api.getPet().then((p) => p && setPetState(p)).catch(() => {});
  }, []);
    ...
    return (
      <section className={cardClasses}>
        <div className="flex items-center gap-2 mb-4">
          <Cat size={20} className="text-gray-700 dark:text-gray-200 midnight:text-gray-200" />
          <h3 className="text-base font-medium text-gray-800 dark:text-gray-100 midnight:text-gray-100">Desktop Pet</h3>
        </div>
  ...
  const apply = async (run, successText) => {
    ...
    try {
      const res = await run();
      if (res && res.success === false) throw new Error(res.error || 'Failed to update pet');
  ...
  const toggleEnabled = () =>
    apply(() => api.setPet({ enabled: !pet.enabled }), pet.enabled ? 'Pet hidden.' : 'Pet is now on screen.');

  const chooseDefault = () => apply(() => api.setPet({ sprite: 'default' }), 'Pet updated.');
  const chooseCustom  = () => apply(() => api.setPet({ sprite: 'custom' }), 'Pet updated.');
  ...
  const uploadCustom = async () => {
    ...
    await apply(() => api.setPet({ sprite: 'custom', path: result.filePaths[0] }), 'Pet updated.');
  };
    ...
    <section className={cardClasses}>
      ...
      <div className={insetClasses}>
        ...
        <label className="flex items-center justify-between gap-4 cursor-pointer">
          <span className="flex flex-col">
            ...
            <span className={mutedClasses}>
              A floating companion that stays on top across windows and shows agent activity.
            </span>
          </span>
          <button
            type="button"
        ...
        <p className={`${mutedClasses} mt-4`}>
          Drag the pet anywhere on screen. It shows a ⚙️ bubble while agents are running and a green
          ✓ when a run finishes. PNG, JPG, WebP, or an animated GIF works as a sprite.
        </p>
      </div>
    </section>
  );
