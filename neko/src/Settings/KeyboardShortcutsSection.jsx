...
const KeyRecorder = ({ shortcut, onSave, onCancel }) => {
  ...
  if (recording) {
    return (
      <div className="flex items-center gap-2">
        ...
        >
          <input type="text" readOnly value={displayKey()} className="bg-transparent text-blue-600 dark:text-blue-400 midnight:text-blue-400 font-mono text-sm w-32" />
          <span className="text-xs text-blue-500 dark:text-blue-400 midnight:text-blue-400 animate-pulse">Recording...</span>
        </div>
        <button onClick={handleSave} className="px-3 py-1.5 rounded-lg bg-blue-600 text-white text-sm hover:bg-blue-700 midnight:bg-blue-800 midnight:hover:bg-blue-700">Save</button>
        <button onClick={handleCancel} className="px-3 py-1.5 rounded-lg bg-gray-200 dark:bg-gray-700 midnight:bg-gray-800 text-gray-700 dark:text-gray-200 midnight:text-gray-200 text-sm hover:bg-gray-300 dark:hover:bg-gray-600 midnight:hover:bg-gray-700">Cancel</button>
      </div>
    );
  }
  ...
  return (
    <button
      ...
      className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-gray-100 dark:bg-gray-800 midnight:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 midnight:hover:bg-gray-700 border border-gray-300 dark:border-gray-600 midnight:border-gray-600 transition-colors"
    >
      <kbd className="font-mono text-xs bg-gray-200 dark:bg-gray-700 midnight:bg-gray-700 px-1.5 py-0.5 rounded">{displayKey()}</kbd>
      <span className="text-xs text-gray-500 dark:text-gray-400 midnight:text-gray-400">Click to change</span>
    </button>
  );
};
    ...
    <div className="space-y-6">
      <div className="backdrop-blur-md bg-white/90 dark:bg-gray-800/90 midnight:bg-gray-900/90 p-6 rounded-xl shadow-sm border border-gray-200/50 dark:border-gray-600/50 midnight:border-gray-500/40">
        <div className="flex items-center justify-between mb-4">
          ...
          <button
            onClick={handleReset}
            className="text-sm text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 midnight:text-gray-400 midnight:hover:text-gray-200"
          >
            Reset to Default
          </button>
        </div>
        ...
        <div className="space-y-3">
          {Object.entries(shortcuts).map(([id, shortcut]) => (
            <div key={id} className="flex items-center justify-between py-2 border-b border-gray-200/50 dark:border-gray-700/50 midnight:border-gray-700/50 last:border-0">
      ...
      {showConflictModal && conflicts.length > 0 && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-white dark:bg-gray-800 midnight:bg-gray-900 rounded-xl shadow-2xl p-6 max-w-md w-full mx-4 border border-gray-200/50 dark:border-gray-700/50 midnight:border-gray-700/50">
            <div className="flex items-center gap-2 mb-4 text-amber-600 dark:text-amber-400 midnight:text-amber-400">
              <AlertTriangle size={20} />
              <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-200 midnight:text-gray-200">Shortcut Conflict</h3>
            </div>

            <p className="text-gray-600 dark:text-gray-300 midnight:text-gray-300 mb-4">
              This key combination is already used for:
            </p>

            <div className="bg-amber-50 dark:bg-amber-900/20 midnight:bg-amber-900/30 rounded-lg p-4 mb-4 border border-amber-200/50 dark:border-amber-700/50 midnight:border-amber-700/50">
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium text-amber-800 dark:text-amber-300 midnight:text-amber-300">{conflicts[0].shortcut.label}</span>
                <kbd className="font-mono text-xs bg-amber-200 dark:bg-amber-800 midnight:bg-amber-900 px-2 py-1 rounded text-amber-900 dark:text-amber-200 midnight:text-amber-200">
            ...
            <div className="flex gap-3 justify-end">
              <button
                ...
                className="px-4 py-2 rounded-lg bg-gray-200 dark:bg-gray-700 midnight:bg-gray-800 text-gray-700 dark:text-gray-200 midnight:text-gray-200 hover:bg-gray-300 dark:hover:bg-gray-600 midnight:hover:bg-gray-700 transition-colors"
              >
                Cancel
              </button>
              <button
                onClick={forceSaveShortcut}
                className="px-4 py-2 rounded-lg bg-amber-600 text-white hover:bg-amber-700 midnight:bg-amber-800 midnight:hover:bg-amber-700 transition-colors"
              >
                Replace
              </button>
            </div>
