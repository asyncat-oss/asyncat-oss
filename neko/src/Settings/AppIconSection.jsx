...
const AppIconSection = () => {
  ...
  const apply = async (run, successText) => {
    ...
    try {
      ...
      if (res && res.success === false)
        throw new Error(res.error || "Failed to update icon");
  ...
  const uploadCustom = async () => {
    ...
    await apply(
      () => api.setAppIcon({ type: "custom", path: result.filePaths[0] }),
      "App icon updated.",
    );
  };

  const reset = () =>
    apply(() => api.resetAppIcon(), "Reset to the default icon.");
    ...
    <section className={cardClasses}>
      ...
      <div className={insetClasses}>
        ...
        <p className={`${mutedClasses} mt-4`}>
          Changes the Dock, window, and tray icons instantly. Uploads are
          cropped to a rounded square to match the app shape. The Finder /
          installer icon is set when the app is built and stays the same. PNG,
          JPG, or WebP — a square 512×512 image works best.
        </p>
      </div>
    </section>
  );
